...
class CartManagerComponent extends Component
{
    ...
    public function initialize(array $config)
    {
        ...
        if (empty($this->getConfig('model'))) {
            $this->setConfig('model', $this->Controller->modelClass);
        }
    ...
    public function initializeCart()
    {
        ...
        $this->CartModel = TableRegistry::get($cartModel);

        if (!$this->Controller->request->getSession()->check($sessionKey)) {
            $this->Controller->request->getSession()->write($sessionKey, $emptyCartStructure);
        } else {
            if ($userId && !$this->Controller->request->getSession()->check($sessionKey . '.Cart.id')) {
                $merged = $this->CartModel->CartsItems->mergeItems(
                    $this->CartModel->getActive($userId),
                    $this->Controller->request->getSession()->read($sessionKey)
                );

                $this->_cartId = $merged['Cart']['id'];
                $this->Controller->request->getSession()->write($sessionKey, $merged);

                foreach ($merged['CartsItem'] as $item) {
                    $this->CartModel->CartsItems->addItem($this->_cartId, $item);
                }
    ...
    public function captureBuy($returnItem = false, $options = array())
    {
        ...
        if (!$data) {
            if ($afterAddItemFailedRedirect === true) {
                $this->Controller->request->getSession()->setFlash(__d('cart', 'Failed to buy the item'));
    ...
    public function itemActionAllowed($type)
    {
        ...
        return (
            $this->getConfig($key) === true ||
            strtoupper($this->getConfig($key)) === $this->Controller->request->method()
        );
    }
    ...
    protected function _additionalData($data, $type = 'update')
    {
        ...
        if (!isset($data['CartsItem']['model'])) {
            $data['CartsItem']['model'] = $this->getConfig('model');
        }
    ...
    public function afterAddItemRedirect($item)
    {
        extract($this->getConfig());
        if ($item === true) {
            $this->Controller->request->getSession()->setFlash(__d('cart', 'Item was successfully removed from your cart'));
        } else {
            $this->Controller->request->getSession()->setFlash(__d('cart', 'You now have {0} {1} in your cart', $item['quantity'], $item['name']));
        }
    ...
    {
        if ($this->Controller->request->is('get') && $this->getConfig('getBuy') === true && isset($this->Controller->request->params['named']['item'])) {
            $data = array(
                'CartsItem' => array(
    ...
    {
        if ($this->Controller->request->is('post') && $this->getConfig('postBuy') === true) {
     ...
     * @param  array $data
     * @param  boolean $recalculate
     * @throws \Cake\Http\Exception\HttpInternalErrorException
    ...
    public function addItem($data, $recalculate = true)
    {
        ...
        /** @var \Cake\ORM\Table $ItemModel */
        $ItemModel = TableRegistry::get($data['CartsItem']['model']);

        $isBuyable = [$ItemModel, 'isBuyable'];
        $beforeAddToCart = [$ItemModel, 'beforeAddToCart'];
        if (!is_callable($isBuyable) || !is_callable($beforeAddToCart)) {
        ...
        if (!call_user_func($isBuyable, $data)) {
            // throw exception?
            return false;
        }

        $data = call_user_func($beforeAddToCart, $data);

        if ($data === false || !is_array($data) || !$this->CartModel->CartsItems->validateItem($data, $this->_isLoggedIn)) {
            $this->validationErrors = $this->CartModel->CartsItems->invalidFields;
            if (Configure::read('debug') > 0 && !empty($this->CartModel->CartsItems->invalidFields)) {
    ...
    public function emptyCart()
    {
        ...
        if ($this->getConfig('useCookie') === true) {
    ...
    {
        return (bool)$this->Controller->request->getSession()->read($this->getConfig('sessionKey') . '.Cart.requires_shipping');
    }
    ...
    {
        $cartData = $this->Controller->request->getSession()->read($this->getConfig('sessionKey'));
        ...
        $this->Controller->request->getSession()->write($this->getConfig('sessionKey'), $cartData);
        return $cartData;
    }
    ...
    {
        return $this->Cookie->write($this->getConfig('cookieName'), $this->content());
    }
    ...
    {
        $result = $this->Cookie->read($this->getConfig('cookieName'));
        $this->Controller->request->getSession()->write($this->getConfig('sessionKey'), $result);
        return (!empty($result));
    }
