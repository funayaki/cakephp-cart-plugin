...
class CartManagerComponent extends Component
{
    ...
    public function initialize(array $config)
    {
        ...
        if (empty($this->getConfig('model'))) {
            $this->setConfig('model', $this->Controller->loadModel()->getAlias());
        }

        $this->sessionKey = $this->getConfig('sessionKey');
    }
    ...
    {
        if ($this->Controller->request->is('get') && $this->getConfig('getBuy') === true && $this->Controller->request->query('item') !== null) {
            $data = array(
                'CartsItem' => array(
                    'foreign_key' => $this->Controller->request->query('item')));

            if ($this->Controller->request->query('model') !== null) {
                $data['CartsItem']['model'] = $this->Controller->request->query('model');
            }

            if ($this->Controller->request->query('quantity') !== null) {
                $data['CartsItem']['quantity'] = $this->Controller->request->query('quantity');
            }
            foreach (array('update', 'increment', 'decrement', 'remove') as $type) {
                if ($this->Controller->request->query($type) !== null) {
